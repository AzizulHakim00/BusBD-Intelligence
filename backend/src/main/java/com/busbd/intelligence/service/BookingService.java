...
public class BookingService {
    ...
    public BookingService(PlatformStore store, SeatLockService seatLocks, TicketService tickets,
        ...
        this.store = store; this.seatLocks = seatLocks; this.tickets = tickets;
        this.fullRefundHours = fullRefundHours; this.partialRefundHours = partialRefundHours;
    }
    ...
    public Map<String, Object> seatMap(UUID tripId) {
        ...
        for (int i = 1; i <= bus.getSeatCount(); i++) {
            ...
            item.put("number", seat); item.put("status", status); item.put("row", coordinate.row());
            item.put("column", coordinate.column()); item.put("aisleAfter", coordinate.aisleAfter());
            item.put("category", women.contains(seat) ? "WOMEN_RESERVED" : bus.getCoachType().toLowerCase().contains("sleeper") ? "SLEEPER" : "REGULAR");
            item.put("price", trip.getFare()); seats.add(item);
        }
        Map<String, Object> view = new LinkedHashMap<>();
        view.put("tripId", tripId); view.put("coachType", bus.getCoachType()); view.put("seatLayout", bus.getSeatLayout());
        view.put("seatCount", bus.getSeatCount()); view.put("lockMinutes", 5); view.put("seats", seats);
        return view;
    }
    ...
    public SeatLockService.SeatHold hold(UUID tripId, Collection<String> seats, String owner) {
        ...
        store.reservationsForTrip(tripId).forEach(r -> booked.add(r.getSeatNumber()));
        seats.stream().map(s -> s.toUpperCase(Locale.ROOT)).filter(booked::contains).findFirst().ifPresent(s -> { throw new IllegalStateException("Seat " + s + " has already been booked"); });
        ...
        seats.stream().map(s -> s.toUpperCase(Locale.ROOT)).filter(s -> !valid.contains(s)).findFirst().ifPresent(s -> { throw new IllegalArgumentException("Seat " + s + " does not exist on this bus"); });
    ...
    public Map<String, Object> confirm(BookingCommand command) {
        ...
        SeatLockService.SeatHold hold = seatLocks.get(command.holdId()).orElseThrow(() -> new IllegalStateException("Seat hold expired. Please select seats again."));
        ...
        hold.seats().stream().filter(booked::contains).findFirst().ifPresent(s -> { throw new IllegalStateException("Seat " + s + " is no longer available"); });
        ...
        Booking booking = new Booking(reference, trip.getId(), command.passengerName(), command.passengerEmail(), command.passengerPhone(), String.join(",", hold.seats()), boarding, dropping, subtotal, discount, total);
        store.userByEmail(command.passengerEmail()).ifPresent(u -> booking.setUserId(u.getId()));
        booking.setPromoCode(blankToNull(command.promoCode())); booking.setIdempotencyKey(blankToNull(command.idempotencyKey()));
        booking.setPaymentProvider(Optional.ofNullable(command.paymentProvider()).filter(s -> !s.isBlank()).orElse("MOCK"));
        booking.setPaymentStatus("PAID"); booking.setPaymentReference("PAY-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
        store.save(booking); store.flush();
        ...
        for (PassengerCommand passenger : passengers) {
            store.save(new BookingPassenger(booking.getId(), passenger.fullName(), normalizeType(passenger.passengerType()), normalizeGender(passenger.gender()), passenger.seatNumber().toUpperCase(Locale.ROOT), passenger.phone()));
        }
        booking.setQrPayload(tickets.issue(booking)); store.save(booking); store.flush();
        trip.setAvailableSeats(Math.max(0, trip.getAvailableSeats() - hold.seats().size())); store.save(trip);
        store.save(new AuditLog(command.passengerEmail(), "BOOKING_CONFIRMED", "Booking", reference, "Seats=" + booking.getSeatNumbers() + ", amount=" + total + ", boarding=" + boarding + ", dropping=" + dropping));
        seatLocks.consume(command.holdId());
        return bookingView(booking);
    }

    public Map<String, Object> booking(String reference) { return bookingView(store.bookingByReference(reference).orElseThrow(() -> new NoSuchElementException("Booking not found"))); }
    public List<Map<String, Object>> bookingsForUser(String email) { return store.bookingsByEmail(email).stream().map(this::bookingView).toList(); }
    ...
    public Map<String, Object> cancel(String reference, String actor, boolean privileged, String reason) {
        ...
        booking.setStatus("CANCELLED"); booking.setCancellationReason(Optional.ofNullable(reason).orElse("Passenger cancellation"));
        booking.setCancelledAt(Instant.now()); booking.setRefundStatus("COMPLETED"); booking.setRefundAmount(refundAmount); store.save(booking);
        store.save(new Refund(booking.getId(), refundAmount, booking.getCancellationReason())); store.deleteReservationsForBooking(booking.getId());
        trip.setAvailableSeats(Math.min(store.bus(trip.getBusId()).orElseThrow().getSeatCount(), trip.getAvailableSeats() + booking.getSeatNumbers().split(",").length)); store.save(trip);
    ...
    public Map<String, Object> verifyTicket(String token) {
        ...
        result.put("bookingStatus", booking.getStatus()); result.put("passengerName", booking.getPassengerName());
        result.put("boardingPoint", booking.getBoardingPoint()); result.put("droppingPoint", booking.getDroppingPoint());
        result.put("travelAllowed", "CONFIRMED".equals(booking.getStatus()) && "PAID".equals(booking.getPaymentStatus())); return result;
    }

    private Map<String, Object> bookingView(Booking b) {
        Trip trip = store.trip(b.getTripId()).orElseThrow(); RoutePlan route = store.route(trip.getRouteId()).orElseThrow();
        Map<String, Object> view = new LinkedHashMap<>();
        view.put("reference", b.getReference()); view.put("tripId", b.getTripId()); view.put("passengerName", b.getPassengerName());
        view.put("passengerEmail", b.getPassengerEmail()); view.put("passengerPhone", b.getPassengerPhone()); view.put("seats", Arrays.asList(b.getSeatNumbers().split(",")));
        view.put("boardingPoint", b.getBoardingPoint()); view.put("droppingPoint", b.getDroppingPoint()); view.put("origin", route.getOrigin()); view.put("destination", route.getDestination());
        view.put("departureTime", trip.getDepartureTime()); view.put("arrivalTime", trip.getArrivalTime()); view.put("subtotal", b.getSubtotal());
        view.put("discountAmount", b.getDiscountAmount()); view.put("totalAmount", b.getTotalAmount()); view.put("promoCode", b.getPromoCode());
        view.put("status", b.getStatus()); view.put("paymentStatus", b.getPaymentStatus()); view.put("paymentProvider", b.getPaymentProvider());
        view.put("paymentReference", b.getPaymentReference()); view.put("refundStatus", b.getRefundStatus()); view.put("refundAmount", b.getRefundAmount());
        view.put("createdAt", b.getCreatedAt()); view.put("passengers", store.passengersForBooking(b.getId())); view.put("ticketToken", b.getQrPayload()); view.put("qrCode", qrDataUri(b.getQrPayload()));
        return view;
    }
    ...
    private List<PassengerCommand> normalizePassengers(BookingCommand command, Set<String> heldSeats) {
        ...
        input.forEach(p -> { if (p.seatNumber() != null) bySeat.put(p.seatNumber().toUpperCase(Locale.ROOT), p); });
        ...
        for (String seat : heldSeats) {
            ...
            result.add(new PassengerCommand(p.fullName().trim(), normalizeType(p.passengerType()), normalizeGender(p.gender()), seat, p.phone()));
        }
        return result;
    }
    private void validateWomenSeats(Bus bus, List<PassengerCommand> passengers) {
        ...
        passengers.stream().filter(p -> womenSeats.contains(p.seatNumber()) && "MALE".equals(p.gender())).findFirst().ifPresent(p -> { throw new IllegalArgumentException("Seat " + p.seatNumber() + " is reserved for women passengers"); });
    }
    private String choosePoint(String requested, String tripDefault, String csv, String routeDefault, String type) {
        String chosen = Optional.ofNullable(requested).filter(s -> !s.isBlank()).orElse(Optional.ofNullable(tripDefault).filter(s -> !s.isBlank()).orElse(routeDefault));
        Set<String> allowed = csvSet(csv);
        if (!allowed.isEmpty() && allowed.stream().noneMatch(p -> p.equalsIgnoreCase(chosen))) throw new IllegalArgumentException("Invalid " + type + " point");
        return chosen;
    }
    ...
    private BigDecimal promoDiscount(BigDecimal subtotal, String promo) { return "BUSBD10".equalsIgnoreCase(Optional.ofNullable(promo).orElse("")) ? subtotal.multiply(new BigDecimal("0.10")) : BigDecimal.ZERO; }
    private String normalizeType(String value) { return "CHILD".equalsIgnoreCase(value) ? "CHILD" : "ADULT"; }
    private String normalizeGender(String value) { if ("FEMALE".equalsIgnoreCase(value)) return "FEMALE"; if ("MALE".equalsIgnoreCase(value)) return "MALE"; return ""; }
    private String blankToNull(String value) { return value == null || value.isBlank() ? null : value.trim(); }
    private String nextReference() {
        for (int i = 0; i < 20; i++) { String reference = "BBD" + (100000 + random.nextInt(900000)); if (store.bookingByReference(reference).isEmpty()) return reference; }
        return "BBD" + System.currentTimeMillis();
    }
    private String qrDataUri(String payload) {
        try { var matrix = new QRCodeWriter().encode(payload, BarcodeFormat.QR_CODE, 240, 240); var out = new ByteArrayOutputStream(); MatrixToImageWriter.writeToStream(matrix, "PNG", out); return "data:image/png;base64," + Base64.getEncoder().encodeToString(out.toByteArray()); }
        catch (Exception e) { return Base64.getEncoder().encodeToString(payload.getBytes(StandardCharsets.UTF_8)); }
    }
    private Set<String> csvSet(String csv) {
        Set<String> result = new LinkedHashSet<>();
        if (csv != null && !csv.isBlank()) Arrays.stream(csv.split(",")).map(String::trim).filter(s -> !s.isBlank()).forEach(result::add);
        return result;
    }
    private SeatCoordinate seatCoordinate(int index, String layout) {
        ...
        int row = (index - 1) / columns + 1; int column = (index - 1) % columns; char letter = (char) ('A' + column);
